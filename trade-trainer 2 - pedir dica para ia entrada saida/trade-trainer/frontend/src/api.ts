...
// PASSO 2: dispara a avaliação por IA para um trade que já tem
// justificativa salva. Pode ser chamado na hora ou depois, no histórico.
export async function runAiEvaluation(tradeId: string) {
  const { data } = await api.post(`/evaluation/${tradeId}/run-ai-evaluation`);
  return data;
}
