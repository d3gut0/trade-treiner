...
export function SessionHistory() {
  ...
  const handleEvaluate = async (tradeId: string) => {
    setEvaluatingTradeId(tradeId);
    try {
      await runAiEvaluation(tradeId);
    } catch {
      // erro fica registrado no backend (avaliacaoStatus = ERRO) -
      // o reload abaixo vai refletir isso na tela
    } finally {
      await load();
      setEvaluatingTradeId(null);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <p style={{ color: '#9ca3af' }}>
        {sessions.length} sessão(ões) registrada(s). Clique em cada uma para ver os trades,
        justificativas e vereditos da IA — e avaliar com IA quando quiser.
      </p>
      ...
      <Accordion multiple>
        {sessions.map((session, idx) => {
          ...
          const pendentes = trades.filter(
            (t) =>
              t.justification &&
              (t.justification.avaliacaoStatus === 'PENDENTE' ||
                t.justification.avaliacaoStatus === 'ERRO'),
          ).length;
            ...
            <AccordionTab
              ...
              header={
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {pendentes > 0 && (
                    <Tag value={`${pendentes} pendente(s) de avaliação`} severity="warning" />
                  )}
                </div>
              }
            >
                  ...
                  >
                    ...
                    {j ? (
                      <>
                        ...
                        {j.avaliacaoStatus === 'AVALIADO' && j.avaliacaoIA ? (
                          ...
                          <div style={{ marginTop: '0.5rem' }}>
                            {j.avaliacaoStatus === 'ERRO' && (
                              <Message
                                severity="error"
                                text={`Última tentativa falhou: ${j.avaliacaoErro ?? 'erro desconhecido'}`}
                                style={{ marginBottom: '0.5rem', width: '100%' }}
                              />
                            )}
                            <Button
                              label={j.avaliacaoStatus === 'ERRO' ? 'Tentar avaliar de novo' : 'Avaliar com IA'}
                              icon="pi pi-sparkles"
                              size="small"
                              onClick={() => handleEvaluate(trade.id)}
                              loading={evaluatingTradeId === trade.id}
                            />
                          </div>
                        )}
                      </>
